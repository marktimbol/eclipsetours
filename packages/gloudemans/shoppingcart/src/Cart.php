...
class Cart {
	...
	protected function updateRow($rowId, $attributes)
	{
		...
		{
			$row->put('subtotal', ($row->qty * $row->price) + ($row->options->child_quantity * $row->options->package->child_price));
		}
	...
	protected function createRow($rowId, $id, $name, $qty, $price, $options)
	{
		...
		$newRow = new CartRowCollection([
			...
			'subtotal' => $qty * $price + $options['child_quantity'] * $options['package']['child_price']
